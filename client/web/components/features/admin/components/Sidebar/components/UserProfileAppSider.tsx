...
import { ChevronRight, Edit, User, Settings, LogOut } from "lucide-react"
...
export default function UserProfileAppSider({
  isCollapsed,
  isMobile,
  ...
  isCollapsed: boolean
  isMobile: boolean
  setIsProfileDialogOpen: (v: boolean) => void
}) {
    ...
    <>
      <DropdownMenuLabel>
        <div className="flex flex-col space-y-1">
          ...
          {displayRole && <p className="text-xs text-muted-foreground/70">Vai trò: {displayRole}</p>}
        </div>
      </DropdownMenuLabel>
      <DropdownMenuSeparator />
          ...
          <Tooltip>
            ...
            <TooltipContent side="right" className="bg-popover text-popover-foreground border">
              <div>
                ...
                {displayRole && <p className="text-xs opacity-75">Vai trò: {displayRole}</p>}
              </div>
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
