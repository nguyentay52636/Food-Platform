...
export default function LoginForm({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!account.trim() || !password.trim()) {
            // Hiển thị lỗi cục bộ nếu rỗng (mặc dù 'required' đã ngăn chặn phần lớn trường hợp)
            return
        }

        dispatch(loginThunk({ account, password }))
    }
            ...
            <FieldGroup className="px-6 py-4">
                <div className="flex flex-col items-center gap-3 text-center mb-8">
                    ...
                    <p className="text-muted-foreground text-base text-balance">
                        Nhập email hoặc tên đăng nhập của bạn để đăng nhập
                    </p>
                </div>
                ...
                <Field className="mb-4">
                    <FieldLabel htmlFor="account" className="text-lg mb-2">Tài khoản</FieldLabel>
                    <Input
                        id="account"
                        type="text"
                ...
                <Field>
                    <Button variant="outline" type="button" className="w-full h-12 text-lg" disabled={isLoading}>
